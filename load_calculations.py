...
def weight_distribution(mass_wing, b, c_r, c_t):
    ...
    A = 3 / 2 * mass_wing*g *np.cos(np.radians(alpha(CL_op))) / (y_0**3-(y_0 - b/2)**3) #It is divided by 2 since we are only considering half the span and thus half of the weight
    w = A * (y_0 - y_arr)**2
    return w
